...
const getSource = (slug, requestedSource, osdBranch) => {
  ...
  if (source) {
    ...
    if (user && repo && branch) return { user, repo, branch };
    if (user && repo === undefined && branch === undefined) return getOfficialSource(slug, branch);
  } else if (osdBranch) return getOfficialSource(slug, osdBranch); else return getOfficialSource(slug);
...
const getOfficialSource = (slug, requestedBranch) => {
  if (!projectSlugs[slug]) throw `Unknown key: ${slug}`;
  const [, , user, repo, branch] = projectSlugs[slug].src.split('/');
...
export const prepareDashboards = async opts => {
  ...
  const osdDir = path.join(destination, `Dashboards-${dirSuffix}`);
